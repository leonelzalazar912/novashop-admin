...
>
> Última actualización: 02/08/2026 (cancelar/borrar una orden ya **revierte el stock** que había descontado `checkout_create_order`, vía las funciones nuevas `update_order_status`/`delete_order` — incluye reactivar una orden cancelada, con validación de stock disponible antes de volver a descontar. De paso se corrigió que `OrdersPage.tsx` no mostraba errores del hook, y que la sección Productos del admin no se actualizaba sola al cambiar stock desde otra sección). Ver `docs/bitacora-desarrollo.txt` para el detalle sesión por sesión.
...
- **`update_order_status(p_order_id, p_store_id, p_status, p_fulfillment_status)`** (agregada el 01-02/08/2026, `SECURITY DEFINER`, `GRANT EXECUTE` solo a `authenticated` — es una operación de admin, no de storefront público): reemplaza a los `update` directos de `orders.status`/`fulfillment_status` que hacía `useOrders.ts`. Verifica `is_store_member(p_store_id)` antes de nada (necesario porque, al ser `SECURITY DEFINER`, bypassea RLS — sin este chequeo cualquier usuario autenticado podría tocar pedidos de otra tienda pasando su `store_id` a mano). Después decide sola, comparando el `status` actual contra el que pide el cliente: si la transición es **hacia** `'cancelled'` desde cualquier otro estado, libera el stock (revierte cada línea de `order_items` a `inventory_levels`, con un `stock_movements` `'order_release'`); si es **desde** `'cancelled'` hacia cualquier estado activo, vuelve a descontar el stock, pero antes valida que haya suficiente disponible (mismo criterio de `checkout_create_order`) — si no alcanza, `raise exception` y no cambia nada. Cualquier otra transición (activo → activo) solo actualiza el estado, sin tocar inventario. Para saber cuánto stock corresponde a cada producto de la orden usa el `stock_movements` más reciente de esa orden por `product_id` (`DISTINCT ON (product_id) ... ORDER BY created_at DESC`) en vez de asumir un único movimiento — así funciona sin importar cuántos ciclos cancelar/reactivar tenga una orden.
- **`delete_order(p_order_id, p_store_id)`** (agregada el 01-02/08/2026, mismas convenciones que `update_order_status`): antes de borrar una orden, libera su stock si todavía no estaba liberado (mismo criterio: compara contra el `status` actual de la orden, no contra el historial de movimientos). El `ON DELETE CASCADE` ya existente en `order_items`/`order_addresses`/`order_status_history`/`payments`/`shipments` se encarga del resto. Reemplaza al `delete` directo que hacía `useOrders.ts`.
...
**Resuelto en la sesión del 31/07/2026 (continuación — auditoría de `schema_dump.sql`):**
- ~~Auditar `schema_dump.sql` contra el estado real de `novashop-development`~~ → auditado por completo (constraints, índices, funciones y triggers) comparando `pg_constraint`/`pg_indexes`/`pg_proc`/`pg_trigger` contra el archivo. **Resultado: todo coincide.** 120 constraints (25 PK + 14 UNIQUE + 27 CHECK + 54 FK), 61 índices (39 auto-creados por PK/UNIQUE + 22 explícitos), 6 funciones y 19 triggers — mismos nombres, mismas definiciones, en ambos lados, sin faltantes ni sobrantes. El constraint `payments_provider_reference_unique` (el que faltaba) se confirmó presente y correcto. Conclusión: el problema de esa constraint fue un caso aislado, no un síntoma de un desfase más amplio entre el dump y la base real — no hace falta seguir auditando el resto del schema por este motivo.

**Resuelto en la sesión del 01-02/08/2026 (revertir stock al cancelar/borrar una orden):**
- ~~Cancelar o borrar una orden no revertía el stock descontado por `checkout_create_order`~~ → resuelto con dos funciones Postgres nuevas, `update_order_status` y `delete_order` (ver § Funciones de Postgres), invocadas desde `useOrders.ts` en vez de los `update`/`delete` directos que tenía antes. Incluye el caso que no estaba en el alcance original y se agregó al confirmarlo con el usuario: **reactivar** una orden cancelada (volverla a un estado activo) vuelve a descontar el stock, pero valida primero que haya suficiente disponible — si no alcanza, rechaza el cambio en vez de dejar stock negativo.
- ~~`OrdersPage.tsx` no mostraba el `error` de `useOrders()`~~ (encontrado al implementar el punto anterior: sin esto, un rechazo por "stock insuficiente para reactivar" se perdía en silencio y el formulario mostraba "Pedido actualizado correctamente" igual) → corregido con el mismo patrón que ya usa `ProductsPage.tsx` (`<Message message={error} />`), y `OrderForm.tsx` ya no limpia el formulario ni cierra la edición cuando la operación falla.
- ~~Sección Productos del admin no reflejaba cambios de stock hechos desde otra sección sin recargar la página~~ (encontrado al probar el punto de arriba: cancelar/reactivar desde Pedidos no se veía en Productos hasta cerrar sesión) → causa: `useProducts()` carga una sola vez al montar `AdminDashboard` y no vuelve a hacerlo nunca, porque `AdminDashboard` no se desmonta al navegar entre secciones. Fix acotado: `useProducts()` expone `reload()`, y `AdminDashboard.tsx` lo llama cada vez que se entra a la sección Productos. No resuelve la falta general de invalidación de caché entre secciones (ver punto 5 de Pendiente) — fue un parche puntual para este caso.

**Pendiente:**

1. `src/config/categories.ts`: el export `categories` (Electrónica/Ropa/Perfumería/Juguetes) confirmado **sin ningún uso** en el código — solo se usa `navigation` de ese mismo archivo. Candidato a limpiar.
2. `handleDeleteProduct` borra `product_images`/`product_suppliers`/`inventory_levels` **antes** de intentar borrar `products` (es el único orden posible del lado cliente, porque esas tablas también bloquearían el borrado del producto si quedaran). Si el borrado final de `products` falla por una causa no cubierta por el pre-chequeo de `ProductsTable.tsx` (ej. una compra creada en el instante exacto entre el chequeo y el borrado), el producto queda "vivo" pero sin sus filas relacionadas. No es atómico porque Supabase-js no soporta transacciones multi-tabla desde el cliente; la solución correcta sería una función Postgres (RPC) que envuelva todo en una transacción — mismo patrón que ya se usó para `checkout_create_order`. Se aceptó el riesgo residual (bajo, single-admin) en vez de construir el RPC.
...
5. Revisar la duplicación entre `use<Feature>.ts` (dentro de cada feature) y `use<Feature>Data.ts` (en `modules/admin/hooks/`), que parecen resolver fetch similar por caminos separados. Relacionado: no hay ninguna librería de data-fetching (React Query/SWR) que invalide cachés entre features automáticamente — el `reload()` de Productos (ver arriba) es un parche puntual, no una solución general; cualquier otra sección que mute datos que otra sección tenga cacheados va a tener el mismo problema.
6. Los hooks de admin acceden a Supabase directo desde el hook, incumpliendo la decisión de arquitectura §6.6 (`Componente → hook → servicio → Supabase`). `users`, `core/catalog` y `core/checkout` son los únicos alineados con la decisión. Evaluar si conviene formalizarlo para el resto o relajar la decisión documentada.
7. Advertencia de bundle >500 kB en cada `npm run build` (preexistente, no abordada — candidato: `import()` dinámico para separar admin del storefront).
9. Modelo de credenciales de pago hoy es de una sola tienda (`MERCADOPAGO_ACCESS_TOKEN` como secreto global). El día que NovaShop se venda a otros comercios, cada tienda va a necesitar su propia cuenta de Mercado Pago y sus propias credenciales — no puede seguir siendo un secreto único. Anotado para cuando se encare el modelo multi-tenant real (ver ProductVision.md § Visión de plataforma comercial).
