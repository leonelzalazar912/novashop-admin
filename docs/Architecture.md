...
>
> Última actualización: 31/07/2026 (Payment Brick de Mercado Pago **validado de punta a punta**: pago aprobado, pago rechazado con reintento sobre la misma orden sin duplicar nada. En el camino se encontraron y corrigieron dos bugs reales — un constraint de `payments` documentado pero nunca aplicado en la base real, y un error silencioso en `mercadopago-create-payment` que dejaba marcar una orden como pagada sin haber podido registrar el pago. Como cierre, se auditó `schema_dump.sql` completo contra la base real y se confirmó que el resto del schema sí estaba sincronizado — el gap del constraint fue un caso aislado). Ver `docs/bitacora-desarrollo.txt` para el detalle sesión por sesión.
...
8. Cancelar una orden desde el admin (`updateOrder` → `status: 'Cancelado'`) no revierte el stock descontado por `checkout_create_order` — gap detectado durante el diseño de la integración de pagos, no resuelto todavía. El schema ya tiene el vocabulario para esto (`stock_movements.movement_type` acepta `order_reservation`/`order_release`) pero no está implementado. Reconfirmado el 31/07/2026: borrar una orden (no solo cancelarla) tampoco revierte el stock ni limpia `stock_movements` — mismo gap aceptado, ahora también documentado para el caso de borrado.
